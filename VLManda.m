function [ard] = VLManda(geo,flc,sim,plotar,varargin)
...
%% ================================= VLM ==================================
%     if beta
%         for i=1:length(flc.beta)
%             fltcond.beta                       = flc.beta(i);          %vetor alpha [deg]
%             fltcond.UpdateRe;
%             for j=1:length(flc.aoa)
%                 fltcond.alpha                    = flc.aoa(j);          %vetor de beta [deg]
%                 
%                 if fltcond.alpha ==0 && wing==true
%                     plane.Aero.Settings.NLVLM.Cl_from_adjusted_2DCurvesEq=0;  %utilizando pontos da curva 2D (o polinomio custuma superestimar para baixos aoa). Mais lento!
%                 else
%                     plane.Aero.Settings.NLVLM.Cl_from_adjusted_2DCurvesEq=1;  %utilizando polinomio 2D. Mais rapido
%                 end
% 
%                 [results] = NL_VLM(plane,fltcond,'-append','-itermax',8);
%                 
%             end
%             
%             for k=1:length(plane.LiftingSurfaces)
%                 ard(k,i).Coeffs              = plane.LiftingSurfaces(k).Aero.Coeffs;
%                 ard(k).MACXYZ                = plane.LiftingSurfaces(k).Geo.MAC_Pos;
%                 ard(k).MAC                   = plane.LiftingSurfaces(k).Geo.MAC;
%             end
%             ard(k+1,i).Coeffs              = plane.Aero.Coeffs;
%             ard(k+1).MACg                  = plane.Geo.MAC;
%             ard(k+1).MACXYZg               = plane.Geo.MAC_Pos;
%             
%             
% 
%             clear plane.LiftingSurfaces.Aero.Coeffs
%         end
%     else
% ============================== SEM BETA =================================
